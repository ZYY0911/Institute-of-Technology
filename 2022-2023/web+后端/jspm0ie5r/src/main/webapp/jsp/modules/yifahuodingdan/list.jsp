...
<body>
		...
		<div id="content">
			...
			<div class="container mt-0">
				<div class="row breadcrumb-bar">
					...
					<div class="col-md-6">
						<ol class="breadcrumb">
							<li class="breadcrumb-item"><a href="${pageContext.request.contextPath}/index.jsp"> <span
										class="ti-home"></span>
								</a></li>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">已发货订单列表</h3>
											...
											<th class="no-sort" style="min-width: 35px;">
												<div class="custom-control custom-checkbox">
													<input class="custom-control-input" type="checkbox" id="select-all"
														onclick="chooseAll()"> <label class="custom-control-label"
														for="select-all"></label>
												</div>
											</th>
											<th onclick="sort('orderid')">订单id<i id="orderidIcon"
													class="fa fa-sort"></i></th>
											<th onclick="sort('userid')">用户id<i id="useridIcon" class="fa fa-sort"></i>
											</th>
											<th onclick="sort('goodid')">商品id<i id="goodidIcon" class="fa fa-sort"></i>
											</th>
											<th onclick="sort('goodname')">商品名称<i id="goodnameIcon"
													class="fa fa-sort"></i></th>
											<th onclick="sort('picture')">商品图片<i id="pictureIcon"
													class="fa fa-sort"></i></th>
											<th onclick="sort('buynumber')">购买数量<i id="buynumberIcon"
													class="fa fa-sort"></i></th>
											<th onclick="sort('price')">价格/积分<i id="priceIcon" class="fa fa-sort"></i>
											</th>
											<th onclick="sort('discountprice')">折扣价格<i id="discountpriceIcon"
													class="fa fa-sort"></i></th>
											<th onclick="sort('total')">总价格/总积分<i id="totalIcon" class="fa fa-sort"></i>
											</th>
											<th onclick="sort('discounttotal')">折扣总价格<i id="discounttotalIcon"
													class="fa fa-sort"></i></th>
											<th onclick="sort('type')">支付类型<i id="typeIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('status')">状态<i id="statusIcon" class="fa fa-sort"></i>
											</th>
											<th onclick="sort('address')">地址<i id="addressIcon" class="fa fa-sort"></i>
											</th>
											<th>操作</th>
										</tr>
									</thead>
	...
	<%@include file = "../../static/toLogin.jsp" %>
		...
		function setDataRow(item, number) {
			...
			if (item.address.length >= 11) {
				addressCell.innerHTML = item.address.substring(0, 10) + '..';
			} else {
			...
			//删除按钮
			var shouhuoBtn = document.createElement('button');
			var shouhuoAttr = 'shouhuo(' + item.id + ')';
			shouhuoBtn.setAttribute("type", "button");
			shouhuoBtn.setAttribute("class", "btn btn-success btn-sm 修改");
			shouhuoBtn.setAttribute("onclick", shouhuoAttr);
			shouhuoBtn.innerHTML = "确认收货"
			btnGroup.appendChild(shouhuoBtn)
	...
	<%@include file = "../../static/toLogin.jsp" %>
		...
		function shouhuo(id) {
			var flag = confirm("请确认是否收货?");
			if (flag) {
				httpJson("orders/update", "POST", {
					id: id,
					status: "已完成"
				}, (res) => {
					if (res.code == 0) {
